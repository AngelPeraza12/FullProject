...
export class AppointmentRepository {
    ...
    public async updateAppointment(id: number, updates: Partial<AppointmentResDB>): Promise<void> {
        try {
            ...
            logger.error('Failed updated citas in repository', { error })
            throw new AppointmentUpdateError()
        }
    }

    public async deleteAppointment(id: number, updates: Partial<AppointmentResDB>): Promise<void> {
        try {
            await db('citas').where({ id_cita: id }).delete()
        } catch (error) {
            logger.error('Failed updated citas in repository', { error })
